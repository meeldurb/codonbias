...
##Script for calculating the CAI of domains of all the genomes
##from the ENA namespace on blazegraph using the iterated weight tables
###################################################################
...
outfolder <- "new_CAI_intradomains_ENA/"  
...
for (genomeID in genome.and.organisms[,1]) { 
  ...
  if (!file.exists(fileout)) {
    ...
    if (file.exists(w.files)){
      ...
      cai.output <- compute.cai(domain.data.CDS, genomeID, w, "tmpintradomain.csv", "tmpintradomain.fasta")
