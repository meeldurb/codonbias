...
for (genomeID in genome.and.organisms[,1]) { 
  cat (genomeID, "\n")
  fileout <- paste(outfolder, genomeID, "_CAI_CDS_new.csv", sep="")
  ...
  if (!file.exists(fileout)) {
    ...
    if (file.exists(w.files)){
      ...
      cai.output <- compute.cai(gene.data, genomeID, w, "tmpCDS.csv", "tmpCDS.fasta")
      ...
      write.table(cai.output, file = fileout, append = F, sep = ",", quote = F, col.names = T, row.names = F)
